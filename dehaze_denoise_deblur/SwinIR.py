...
#         print(f'downloading model {model_path}')
    ...
    folder, save_dir, border, window_size = setup(folder_gt)
...
#     for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
    ...
    imgname, img_lq, img_gt = get_image_pair(folder_gt)  # image to HWC-BGR, float32
...
# #               format(imgname, psnr, ssim, psnrb, psnr_y, ssim_y, psnrb_y))
# #     else:
...
def setup(folder_gt):
    ...
    # 004 grayscale image denoising/ 005 color image denoising
        ...
        save_dir = f'results/swinir_{task}_noise{noise}'
        folder = folder_gt
        border = 0
        window_size = 8
    ...
    # 006 JPEG compression artifact reduction
        ...
        save_dir = f'results/swinir_{task}_jpeg{jpeg}'
        folder = folder_gt
        border = 0
        window_size = 7
